...
import ItemOrderDetail from "@/components/shop/ItemOrderDetail";
...
export default function DetailOrderView() {
  ...
  async function fetchOrder() {
    try {
      ...
      const order = await response.json();
      setFirstDirection(order.firstDirection);
      setSecondDirection(order.secondDirection);
      setDistrict(order.district);
      setCity(order.city);
      setCountry(order.country);
      setPaymentMethod(order.paymentMethod);
      setCardNumber(order.cardNumber);
      setNameOnCard(order.nameOnCard);
      setExpiration(order.expiration);
      setCcv(order.ccv);
      setShippingMethod(order.shippingMethod);
      setDetailsOrder(order.detailsOrder);
      const subTotal = order.detailsOrder.reduce(
        (total, detailOrder) =>
          total + detailOrder.product.price * detailOrder.quantity,
        0
      );
      const shipmentAmount = shippingMethod === "Económico" ? 10 : 17;
      const tax = subTotal * 0.18;
      const total = subTotal + shipmentAmount + tax;
  ...
  return (
    <main className="space-y-4 py-4 px-10">
      <Heading text={`Detalles de Order Nro ${id}`} />
      ...
      <div className="flex gap-x-[37px] justify-between items-center">
        <CheckoutCard className="h-[467px]" heading="Items en Pedido:">
          <div className="pl-[56px] pr-[17px] flex flex-col gap-y-[20px]">
            {detailsOrder.map((detailOrder) => (
              <ItemOrderDetail key={detailOrder.id} detailOrder={detailOrder} />
            ))}
          </div>
        </CheckoutCard>
