...
export default function UpdateSerieView() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const updatedProducts = selectedProducts.map((selectedProduct) => {
        fetch(`http://localhost:4000/api/products/${selectedProduct.id}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  async function fetchSerie() {
    try {
      ...
      setPreview(serie.image);
      setName(serie.name);
      setDescription(serie.description);
      setSelectedProducts(serie.products);
    } catch (error) {
      toast.error("Error al obtener serie");
      navigate("/admin/series");
    }
  }
